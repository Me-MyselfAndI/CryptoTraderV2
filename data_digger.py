...
class DataDigger:
    def __init__(self, file_name, asset_code, prediction_delay, precision=5, dT=5, coef_depth=10):
        ...
        self.prediction_delay = int(round(prediction_delay / dT))
    ...
    def get_data_row(self, for_training=True):
        ...
        if len(self.ask_prices) > self.prediction_delay + self.coef_depth: # + self._threshold_to_start_filling_table:
            ...
            if for_training:
                ask_return_data = [self.ask_prices[-1]]
        ...
        print(f"\n\n\u001b[31mNOT ENOUGH PAST DATA TO GENERATE PADE APPROXIMATIONS. Run "
              f"get_one_row() {self.prediction_delay + self.coef_depth - len(self.ask_prices) + 1}"
