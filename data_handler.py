...
class ModelBuilder:
    ...
    def __init__(self, file, test_ratio=0.3, verbose=0):
        ...
        self.x_train = numpy.expand_dims(self.x_train, axis=1)
        self.x_test = numpy.expand_dims(self.x_test, axis=1)
    ...
    def reshape_row(self, data_row):
        ...
        for col_num in range(int(raw_width / 2) - 2):
            new_row_values.append(data_row[col_num + 1] - data_row[col_num])
    ...
    def reshape_table(self):
        ...
        data_frame = pandas.DataFrame(columns=['buy', 'sell', 'hold'] +
                                              [('A_' + str(i)) for i in range(int(raw_width / 2) - 2)] +
                                              [('B_' + str(i)) for i in range(int(raw_width / 2) - 2)] +
                                              ['A_Pade', 'B_Pade'] +
                                              [('dA_' + str(i)) for i in range(int(raw_width / 2) - 3)] +
                                              [('dB_' + str(i)) for i in range(int(raw_width / 2) - 3)]
                                      )
        ...
        for row_num in range(raw_height):
            ...
            new_row_values.append(1 if delta_price['overall'] > 8.0 else 0)   # whether to buy
            new_row_values.append(1 if delta_price['overall'] < -8.0 else 0)   # whether to sell
            new_row_values.append(1 if -8.0 <= delta_price['overall'] <= 8.0 else 0)    # whether to hold
            ...
            for col_num in range(1, int(raw_width / 2) - 2):
                new_row_values.append(curr_row.iloc[col_num+1] - curr_row.iloc[col_num])
            for col_num in range(int(raw_width / 2), raw_width - 3):
                new_row_values.append(curr_row.iloc[col_num+1] - curr_row.iloc[col_num])
    ...
    def make_model(self, num_folds=10):
        ...
        while attempt < num_folds:
            ...
            curr_x_train = [curr_x_train[i:i+5] for i in range(len(curr_x_train) - 5)]
            curr_x_test = [curr_x_test[i:i+5] for i in range(len(curr_x_test) - 5)]
            curr_y_train = [curr_y_train[i:i+5] for i in range(len(curr_x_train) - 5)]
            curr_y_test = [curr_y_test[i:i+5] for i in range(len(curr_x_test) - 5)]
            # print(list(map(len, (curr_x_train, curr_x_test, curr_y_train, curr_y_test))))
            # Create a model
            neurons_per_layer = 50  # random.randint(200, 1200)
            model = Sequential([
                #Dense(400, input_shape=curr_x_train.shape, activation='relu'),
                #Dropout(0.05),
                #Dense(400, input_dim=400, activation='relu'),
                #Dropout(0.05),
                #Dense(400, input_dim=400, activation='relu'),
                #Dense(3, input_dim=400, activation='softmax')
                LSTM(neurons_per_layer, input_shape=(5, len(curr_x_train[0])), activation='relu', return_sequences=True),
                Dropout(0.05),
            ...
            #              metrics=['accuracy'])
            opt = Adam(learning_rate=0.0001, decay=0.0001)
            model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])
            model.fit(curr_x_train, curr_y_train, epochs=random.randint(100, 400),
                      batch_size=random.randint(50, 600), verbose=self.verbose)
...
if __name__ == "__main__":
    ...
    results = {"good": 0, "poor": 0}
    log = []
    i = 0
    while True:
        i += 1
        try:
            print(f"Trial: {i} - Results so far: \u001b[32m{results['good']} \u001b[31m{results['poor']}\u001b[0m")
            ...
            if len(log) >= 20:
                ...
                if past_predictions[0] == past_predictions.max() and past_data[1] < price_data[-13] - 8.0 or \
                    past_predictions[1] == past_predictions.max() and past_data[1] > price_data[-13] + 8.0 or \
                    past_predictions[2] == past_predictions.max() and price_data[-13] - 8.0 < past_data[1] < price_data[-13] + 8.0:
