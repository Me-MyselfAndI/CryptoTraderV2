...
class ModelBuilder:
    ...
    def __init__(self, file, dimension, test_ratio=0.1):
        ...
        self.x_train, self.x_test, self.y_train, self.y_test = train_test_split(self.data_frame.to_numpy(), y, test_size=test_ratio)
    ...
    """
    def make_model (self, num_folds=25):
        ...
        for attempt in range (num_folds):
            curr_x_train, curr_x_test, curr_y_train, curr_y_test = train_test_split(self.x_train, self.y_train, test_size=0.1)
            # Create a model
            ...
            error = float('inf')
            history = model.fit(curr_x_train, curr_y_train, epochs=10000,
            ...
            result = [(result[i][0], curr_y_test.tolist()[i],
                       abs(result[i][0] - curr_y_test.tolist()[i]) / (curr_y_test.tolist()[i]) if curr_y_test.tolist()[i] != 0 else 0) for i in range(len(curr_y_test))]
